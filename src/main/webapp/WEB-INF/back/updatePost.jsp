                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-9">
                                    <form role="form" action="${ctx}/back/updatePost" method="post" >
                                    	<input type="hidden" name="id" value="${post.id}"/>
                                        <div class="form-group">
                                            <label>文章标题</label>
                                            <input name="postTitle" class="form-control" value="${post.postTitle }"/>
                                        </div>
                                        <div class="form-group">
                                            <label>文章URL</label>
                                            <input name="url" class="form-control" value="${post.url }"/>
                                        </div>
                                        <div class="form-group">
                                            <label>标签, 多个请以分号相隔</label>
                                            <input name="tags" class="form-control" value="${tagnames }"/>
                                        </div>                                        
                                         <div class="form-group">
                                            <label>文章分类</label>
                                            <select name="termId" class="form-control">
                                         	<c:forEach items="${terms}" var="term">
                                                <option value="${term.id}" <c:if test="${post.termId eq term.id}">selected</c:if>>
                                                	${term.name}
                                                </option>
                                            </c:forEach>    
                                            </select>
